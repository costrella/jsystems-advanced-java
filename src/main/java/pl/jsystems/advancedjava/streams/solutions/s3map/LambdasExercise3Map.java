...
import pl.jsystems.advancedjava.streams.solutions.s3map.receivers.CargoLoadedMessageReceiver;
import pl.jsystems.advancedjava.streams.solutions.s3map.receivers.CargoUnloadedMessageReceiver;
import pl.jsystems.advancedjava.streams.solutions.s3map.receivers.GPSTrackingMessageReceiver;
import pl.jsystems.advancedjava.streams.solutions.s3map.repositories.CargoLoadedMessageRepository;
import pl.jsystems.advancedjava.streams.solutions.s3map.repositories.CargoUnloadedMessageRepository;
import pl.jsystems.advancedjava.streams.solutions.s3map.repositories.GPSTrackingMessageRepository;
...
class LambdasExercise3Map
{
    ...
    private void run()
    {
        ...
        String cargosBiggerThan1000kgs = cargoLoadedMessages.stream()
                .map(Message::content)
                .map(CargoLoadedMessageContent::getCargoLoadInKgs)
                .filter(weight -> weight.compareTo(BigDecimal.valueOf(1000)) > 0)
                .map(BigDecimal::toPlainString)
                .collect(Collectors.joining(", "));
        LOGGER.info("All loaded cargo bigger than 1000kg: {}", cargosBiggerThan1000kgs);

        Long cargosBiggerThan2500kgLoadedInLessThan10MinsCount = cargoLoadedMessages.stream()
                .map(Message::content)
                .filter(content -> content.getCargoLoadInKgs().compareTo(BigDecimal.valueOf(2500)) > 0)
                .filter(content -> content.getLoadingTimeTakenInSeconds() < 600)
                .count();
        LOGGER.info("Big cargo loaded fast (count): {}", cargosBiggerThan2500kgLoadedInLessThan10MinsCount);
    }
