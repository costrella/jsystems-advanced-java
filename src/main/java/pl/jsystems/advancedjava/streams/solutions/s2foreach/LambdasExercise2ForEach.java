...
import pl.jsystems.advancedjava.streams.solutions.s2foreach.contents.CargoLoadedMessageContent;
import pl.jsystems.advancedjava.streams.solutions.s2foreach.message.Message;
import pl.jsystems.advancedjava.streams.solutions.s2foreach.receivers.CargoLoadedMessageReceiver;
import pl.jsystems.advancedjava.streams.solutions.s2foreach.receivers.CargoUnloadedMessageReceiver;
import pl.jsystems.advancedjava.streams.solutions.s2foreach.receivers.GPSTrackingMessageReceiver;
import pl.jsystems.advancedjava.streams.solutions.s2foreach.repositories.CargoLoadedMessageRepository;
import pl.jsystems.advancedjava.streams.solutions.s2foreach.repositories.CargoUnloadedMessageRepository;
import pl.jsystems.advancedjava.streams.solutions.s2foreach.repositories.GPSTrackingMessageRepository;
...
class LambdasExercise2ForEach
{
    ...
    private void run()
    {
        ...
        messagesInLast10Minutes.forEach(messageLogger::logReceived);

        cargoLoadedMessages.stream()
                .filter(message -> message.content().getCargoLoadInKgs().compareTo(BigDecimal.valueOf(2000)) > 0)
                .forEach(messageLogger::logReceived);
    }
